...
router.get("/categories", async (req, res) => {
  res.json(await db.prepare("SELECT * FROM property_categories ORDER BY name").all());
});
...
router.get("/agents", async (req, res) => {
  const agents = await db.prepare(`
    SELECT u.id, u.full_name as name, u.company, u.email, u.phone, u.avatar_url as logo,
      ...
      ROUND(CAST(AVG(r.rating) AS NUMERIC), 1) as avg_rating, COUNT(r.id) as review_count
    FROM users u
    LEFT JOIN properties p ON p.user_id = u.id AND p.status = 'active'
    LEFT JOIN reviews r ON r.agent_id = u.id
    WHERE u.is_agent = 1
    GROUP BY u.id, u.full_name, u.company, u.email, u.phone, u.avatar_url, u.bio, u.is_verified, u.website, u.created_at
...
router.get("/agents/:id", async (req, res) => {
  ...
  const stats = await db.prepare("SELECT AVG(rating) as avg, COUNT(*) as count FROM reviews WHERE agent_id = ?").get(req.params.id);
...
router.put("/agents/:id/verify", requireAdmin, async (req, res) => {
  const { is_verified } = req.body;
  const user = await db.prepare("SELECT id, is_agent FROM users WHERE id = ?").get(req.params.id);
  if (!user || !user.is_agent) return res.status(404).json({ message: "Agent not found" });
  await db.prepare("UPDATE users SET is_verified=?, updated_at=? WHERE id=?").run(is_verified ? 1 : 0, new Date().toISOString(), req.params.id);
...
router.post("/agents/:id/reviews", requireAuth, async (req, res) => {
  ...
  const existing = await db.prepare("SELECT id FROM reviews WHERE agent_id=? AND reviewer_id=?").get(req.params.id, req.user.id);
  if (existing) return res.status(409).json({ message: "Already reviewed" });
  await db.prepare("INSERT INTO reviews (id, agent_id, reviewer_id, rating, comment, created_at) VALUES (?,?,?,?,?,?)")
...
router.get("/stats", async (req, res) => {
  const properties = (await db.prepare("SELECT COUNT(*) as c FROM properties WHERE status='active'").get()).c;
  const agents     = (await db.prepare("SELECT COUNT(*) as c FROM users WHERE is_agent=1").get()).c;
  const locations  = (await db.prepare("SELECT COUNT(*) as c FROM locations").get()).c;
  res.json({ properties, agents, locations });
});

// ─── Favorites ────────────────────────────────────────────────────────────────
router.get("/favorites", requireAuth, async (req, res) => {
  const rows = await db.prepare(`SELECT p.* FROM favorites f JOIN properties p ON p.id=f.property_id WHERE f.user_id=? ORDER BY f.created_at DESC`).all(req.user.id);
  const enriched = await Promise.all(rows.map(async (p) => ({
    ...p, images: JSON.parse(p.images||"[]"), features: JSON.parse(p.features||"[]"),
    is_featured: !!p.is_featured, is_new_project: !!p.is_new_project,
    category: p.category_id ? await db.prepare("SELECT * FROM property_categories WHERE id=?").get(p.category_id) : null,
    location: p.location_id ? await db.prepare("SELECT * FROM locations WHERE id=?").get(p.location_id) : null,
  })));
  res.json(enriched);
});

router.get("/favorites/:propertyId", requireAuth, async (req, res) => {
  const row = await db.prepare("SELECT id FROM favorites WHERE user_id=? AND property_id=?").get(req.user.id, req.params.propertyId);
...
router.post("/favorites", requireAuth, async (req, res) => {
  ...
  const existing = await db.prepare("SELECT id FROM favorites WHERE user_id=? AND property_id=?").get(req.user.id, property_id);
  if (existing) return res.status(409).json({ message: "Already favorited" });
  await db.prepare("INSERT INTO favorites (id,user_id,property_id,created_at) VALUES (?,?,?,?)").run(uuidv4(), req.user.id, property_id, new Date().toISOString());
  res.status(201).send();
});

router.delete("/favorites/:propertyId", requireAuth, async (req, res) => {
  await db.prepare("DELETE FROM favorites WHERE user_id=? AND property_id=?").run(req.user.id, req.params.propertyId);
  res.status(204).send();
});
...
router.post("/inquiries", async (req, res) => {
  ...
  await db.prepare("INSERT INTO inquiries (id,property_id,name,email,phone,message,created_at) VALUES (?,?,?,?,?,?,?)")
    .run(uuidv4(), property_id, name, email, phone||null, message, new Date().toISOString());

  const prop = await db.prepare("SELECT * FROM properties WHERE id=?").get(property_id);
  if (prop?.user_id) {
...
router.get("/inquiries", requireAuth, async (req, res) => {
  ...
  res.json(await db.prepare("SELECT * FROM inquiries ORDER BY created_at DESC").all());
});

// ─── Notifications ────────────────────────────────────────────────────────────
router.get("/notifications", requireAuth, async (req, res) => {
  res.json(await db.prepare("SELECT * FROM notifications WHERE user_id=? ORDER BY created_at DESC LIMIT 20").all(req.user.id));
});
...
router.get("/plans", async (req, res) => {
  const plans = await db.prepare("SELECT * FROM subscription_plans WHERE is_active=1 ORDER BY price ASC").all();
  res.json(plans.map(p => ({ ...p, features: JSON.parse(p.features || "[]") })));
});

// ─── Saved searches ───────────────────────────────────────────────────────────
router.get("/saved-searches", requireAuth, async (req, res) => {
  const searches = await db.prepare("SELECT * FROM saved_searches WHERE user_id=? ORDER BY created_at DESC").all(req.user.id);
...
router.post("/saved-searches", requireAuth, async (req, res) => {
  ...
  await db.prepare("INSERT INTO saved_searches (id,user_id,name,filters,notify,created_at) VALUES (?,?,?,?,?,?)")
...
router.delete("/saved-searches/:id", requireAuth, async (req, res) => {
  await db.prepare("DELETE FROM saved_searches WHERE id=? AND user_id=?").run(req.params.id, req.user.id);
  res.status(204).send();
});
