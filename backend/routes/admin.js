...
router.get("/profiles", requireAdmin, async (req, res) => {
  const users = await db.prepare("SELECT id, email, full_name, phone, avatar_url, company, is_agent, is_verified, created_at FROM users ORDER BY created_at DESC").all();
...
router.put("/agents/:id/verify", requireAdmin, async (req, res) => {
  ...
  await db.prepare("UPDATE users SET is_verified = ?, updated_at = ? WHERE id = ? AND is_agent = 1")
...
router.get("/payments", requireAdmin, async (req, res) => {
  res.json(await db.prepare("SELECT * FROM payments ORDER BY created_at DESC").all());
});
...
router.get("/user-roles", requireAdmin, async (req, res) => {
  res.json(await db.prepare("SELECT * FROM user_roles ORDER BY created_at DESC").all());
});

// GET /api/admin/wallets
router.get("/wallets", requireAdmin, async (req, res) => {
  res.json(await db.prepare("SELECT * FROM wallets ORDER BY created_at DESC").all());
});

// GET /api/admin/wallet-transactions
router.get("/wallet-transactions", requireAdmin, async (req, res) => {
  res.json(await db.prepare("SELECT * FROM wallet_transactions ORDER BY created_at DESC LIMIT 100").all());
});

// GET /api/admin/commission-config
router.get("/commission-config", requireAdmin, async (req, res) => {
  res.json(await db.prepare("SELECT * FROM commission_config ORDER BY created_at DESC").all());
});
