...
function publicUser(user) {
  return {
    ...
    is_agent: !!user.is_agent, is_verified: !!user.is_verified, role: user.role || "user",
  };
}
...
router.post("/register", authLimiter, async (req, res) => {
  ...
  await db.prepare("INSERT INTO users (id, email, password, full_name, created_at, updated_at) VALUES (?,?,?,?,?,?)")
    .run(id, email.toLowerCase(), hash, full_name || null, now, now);
  await db.prepare("INSERT INTO user_roles (id, user_id, role, created_at) VALUES (?,?,'user',?)").run(uuidv4(), id, now);
  await db.prepare("INSERT INTO wallets (id, user_id, balance, locked_balance, created_at, updated_at) VALUES (?,?,0,0,?,?)").run(uuidv4(), id, now, now);
...
router.put("/profile", requireAuth, async (req, res) => {
  ...
  const now = new Date().toISOString();
  await db.prepare("UPDATE users SET full_name=?,phone=?,avatar_url=?,company=?,bio=?,website=?,is_agent=?,updated_at=? WHERE id=?")
...
router.post("/reset-password", resetLimiter, async (req, res) => {
  ...
  const now = new Date().toISOString();
  await db.prepare("UPDATE users SET password=?, updated_at=? WHERE id=?").run(hash, now, reset.user_id);
